...
test('Brickmerge route stores misses with the right TTL and serves hits without upstream', async () => {
  const originalFetch = globalThis.fetch;
  const originalCaches = globalThis.caches;
  const html = `<html><head><title>LEGO 42154 Ford GT</title></head><body>
    <div id="offerlist">
      <span class="price lowest">96,99 &euro;</span>
      <span class="price">99,95 &euro;</span>
    </div></body></html>`;

  try {
    // 1. MISS - Upstream wird gefragt, das Ergebnis landet im Edge-Cache.
    const stored = [];
    let upstreamCalls = 0;
    ...
    globalThis.caches = {
      default: {
        async match() { return undefined; },
        async put(key, response) { stored.push({ key, response }); }
      }
    };

    const miss = await worker.fetch(
      new Request('https://getdata.example/brickmerge?set=42154'),
      {},
      context
    );
    assert.equal(miss.status, 200);
    assert.equal(miss.headers.get('x-worker-cache'), 'MISS');
    assert.equal(miss.headers.get('cache-control'), 'no-store');
    const missBody = await miss.json();
    assert.equal(missBody.found, true);
    assert.equal(missBody.cheapest.price, 96.99);
    assert.equal(upstreamCalls, 1);

    // Der Cache-Eintrag traegt den oeffentlichen TTL, die Client-Antwort nicht.
    assert.equal(stored.length, 1);
    assert.match(stored[0].key.url, /\/__cache\/brickmerge-price-v1\?set=42154$/);
    assert.equal(
      stored[0].response.headers.get('cache-control'),
      `public, max-age=${2 * 60 * 60}`
    );

    // 2. HIT - die gecachte Antwort kommt ohne Upstream-Abruf zurueck.
    globalThis.caches = {
      default: {
        async match() { return stored[0].response; },
        async put() { throw new Error('HIT darf nicht erneut in den Cache schreiben'); }
      }
    };
    ...
    assert.equal((await hit.json()).cheapest.price, 96.99);
    assert.equal(upstreamCalls, 1, 'HIT darf keinen Upstream-Abruf ausloesen');

    // 3. MISS ohne Treffer - 404 mit dem kuerzeren Leer-TTL.
    const emptyStored = [];
    globalThis.fetch = async () => new Response('<div id="offerlist"></div>', { status: 200 });
    globalThis.caches = {
      default: {
        async match() { return undefined; },
        async put(key, response) { emptyStored.push(response); }
      }
    };
    ...
    assert.equal(empty.headers.get('x-worker-cache'), 'MISS');
    assert.equal((await empty.json()).found, false);
    assert.equal(
      emptyStored[0].headers.get('cache-control'),
      `public, max-age=${20 * 60}`
    );
  } finally {
